...
// Type: FTOptix.Recipe.RecipeSchema
...
namespace FTOptix.Recipe
{
  ...
  public class RecipeSchema : UAObject
  {
    ...
    {
      get => (string) this.Refs.GetVariable(nameof (TableName))?.GetValue();
      set
      {
        if (value != null)
          (this.Refs.GetVariable(nameof (TableName)) ?? this.GetOrCreateVariable(nameof (TableName))).SetValue((object) value);
        else
          this.Refs.GetVariable(nameof (TableName))?.Delete();
      }
    }
    ...
    public event EventHandler<RecipeApplicationEvent> OnRecipeApplication
    {
      add
      {
        this.GetOrCreateCustomEventHandler(new NodeId(NamespaceMapProvider.GetNamespaceIndex("urn:FTOptix:Recipe"), 73U), typeof (EventHandler<RecipeApplicationEvent>), typeof (RecipeApplicationEvent)).Add((Delegate) value);
      }
      remove
      {
        this.GetOrCreateCustomEventHandler(new NodeId(NamespaceMapProvider.GetNamespaceIndex("urn:FTOptix:Recipe"), 73U), typeof (EventHandler<RecipeApplicationEvent>), typeof (RecipeApplicationEvent)).Remove((Delegate) value);
      }
    }
  }
}
