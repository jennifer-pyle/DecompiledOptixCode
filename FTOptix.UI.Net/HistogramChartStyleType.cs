...
// Type: FTOptix.UI.HistogramChartStyleType
...
{
  [MapType(NamespaceUri = "urn:FTOptix:UI", Number = 1085)]
  public class HistogramChartStyleType : ControlStyleType
  {
    public float Spacing
    {
      get => (float) this.GetOptionalVariableValue(nameof (Spacing));
      set => this.SetOptionalVariableValue(nameof (Spacing), new UAValue(value));
    }

    public IUAVariable SpacingVariable => this.GetOrCreateVariable("Spacing");
    ...
    {
      get => new Color(this.GetOptionalVariableValue(nameof (AxisColor)));
      set => this.SetOptionalVariableValue(nameof (AxisColor), new UAValue(value.ARGB));
    }

    public IUAVariable AxisColorVariable => this.GetOrCreateVariable("AxisColor");
    ...
    public InstanceNodeCollection<HistogramChartStyle> Instances
    {
      get => new InstanceNodeCollection<HistogramChartStyle>((IUANode) this);
    }
  }
}
