...
namespace FTOptix.UI
{
  ...
  public class Switch : Item
  {
    ...
    public string CheckedText
    {
      get
      {
        LocalizedText localizedText = (LocalizedText) this.GetOrCreateVariable(nameof (CheckedText)).Value;
      ...
      set
      {
        this.SetOptionalVariableValue(nameof (CheckedText), new UAValue(new LocalizedText(NodeId.InvalidNamespaceIndex, string.Empty, value, this.Context.Sessions.CurrentSessionInfo.ActualLocaleId)));
      }
    }

    public LocalizedText CheckedLocalizedText
    {
      get => (LocalizedText) this.GetOptionalVariableValue("CheckedText");
      set => this.SetOptionalVariableValue("CheckedText", new UAValue(value));
    }

    public UAVariable CheckedTextVariable => (UAVariable) this.GetOrCreateVariable("CheckedText");

    public string UncheckedText
    {
      get
      {
        LocalizedText localizedText = (LocalizedText) this.GetOrCreateVariable(nameof (UncheckedText)).Value;
        if (localizedText.HasTextId)
          return this.Context.LookupTranslation(localizedText).Text;
        return localizedText.HasTranslation ? localizedText.Text : string.Empty;
      }
      set
      {
        this.SetOptionalVariableValue(nameof (UncheckedText), new UAValue(new LocalizedText(NodeId.InvalidNamespaceIndex, string.Empty, value, this.Context.Sessions.CurrentSessionInfo.ActualLocaleId)));
      }
    }

    public LocalizedText UncheckedLocalizedText
    {
      get => (LocalizedText) this.GetOptionalVariableValue("UncheckedText");
      set => this.SetOptionalVariableValue("UncheckedText", new UAValue(value));
    }

    public UAVariable UncheckedTextVariable
    {
      get => (UAVariable) this.GetOrCreateVariable("UncheckedText");
    }
